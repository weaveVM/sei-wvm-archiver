...
pub async fn ps_archive_block(
    ...
    let res = query(
        "INSERT INTO WeaveVMArchiverSei(NetworkBlockId, WeaveVMArchiveTxid) VALUES($0, \"$1\")",
    )
...
pub async fn ps_get_latest_block_id() -> u64 {
    ...
    let latest_archived: u64 =
        query("SELECT MAX(NetworkBlockId) AS LatestNetworkBlockId FROM WeaveVMArchiverSei;")
            .fetch_scalar(&conn)
            .await
...
pub async fn ps_get_archived_block_txid(id: u64) -> Value {
    ...
    let query_formatted = format!(
        "SELECT WeaveVMArchiveTxid FROM WeaveVMArchiverSei WHERE NetworkBlockId = {}",
        id
    );
...
pub async fn ps_get_blocks_extremes(extreme: &str) -> Value {
    ...
    let query_formatted = format!(
        "SELECT NetworkBlockId FROM WeaveVMArchiverSei ORDER BY NetworkBlockId {} LIMIT 1;",
        query_type
    );
...
pub async fn ps_get_archived_blocks_count() -> PsGetTotalBlocksCount {
    ...
    let query_formatted = "SELECT MAX(Id) FROM WeaveVMArchiverSei;";
